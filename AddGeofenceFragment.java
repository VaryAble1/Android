...
// Check if the user entered valid data.
            if (dataIsValid()) {
// If so, create a new NamedGeofence and set its properties. Create a named geofence
...
// Calls the negative click callback on the listener if it exists. Call listener and dismiss or show error
              if (listener != null) {
                listener.onDialogPositiveClick(AddGeofenceFragment.this, geofence);
                dialog.dismiss();
              }
            } else {
// If the user entered invalid data, show a validation error toast. Display an error message
